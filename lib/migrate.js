    ...
    switch (attrName) {
        ...
        case DataTypes.ARRAY.key:
          ...
          if(_type === 'INTEGER[]' || _type === 'STRING[]') {
            ...
            arrayType = (col.type === 'Sequelize.ARRAY(Sequelize.INTEGER)') ? prefix + 'INTEGER' : prefix + 'STRING';
          }
        ...
        for (let column in attributes)
        {
            ...
            if(typeof attributes[column]['type'] === 'undefined')
            {
              if(!attributes[column]['type'])
              {
                ...
                if(!['Sequelize.ARRAY(Sequelize.INTEGER)', 'Sequelize.ARRAY(Sequelize.STRING)'].includes(attributes[column]['type'])) {
                  delete attributes[column];
                  continue;
                }
            ...
            let type = reverseSequelizeColType(attributes[column]);

            // NO virtual types in migration
            if (type === 'Sequelize.VIRTUAL')
            {
            ...
            if (!type)
            {
                if(typeof attributes[column]['type']['options'] !== 'undefined' && typeof attributes[column]['type']['options'].toString === 'function')
                    type = attributes[column]['type']['options'].toString(sequelize);

                if(typeof attributes[column]['type'].toString === 'function')
                    type = attributes[column]['type'].toString(sequelize);
            }

            attributes[column]['type'] = type;
